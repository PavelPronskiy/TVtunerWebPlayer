...
jQuery.fn.bgColor = function(test,value) {
	...
	if (bgColor == '' || bgColor == 'transparent') bgColor = "rgb(000,000,000)";
	return bgColor;
};


// text
jQuery.fn.elementColor = function(test,value) {
	var self = jQuery(this), elementColor;
	if (!test || test.constructor != Function) test = function(e) { return false; };
	if (!value || value.constructor != Function) value = function(e) { return jQuery(e).css('color'); };
	while (true) {
		elementColor = self.css('color');
		if (elementColor != '') break;
		if (test(self[0])) {
			elementColor = value(self[0]);
			break;
		}
		if (self.parent()[0] == document) break;	
		else self = self.parent();
	};
	if (elementColor == '') elementColor = "rgb(255,255,255)";
	return elementColor;
};
